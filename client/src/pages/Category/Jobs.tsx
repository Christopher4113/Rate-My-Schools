...
const Jobs = () => {
  ...
  useEffect(() => {
    axios.get<Form[]>(`http://localhost:8080/auth/getJobs/${id}`)
         .then((response) => {
  ...
  return (
    <div className='min-h-screen flex flex-col'>
      ...
      <div className='flex flex-col md:flex-row p-6 gap-6'>
        <div className='md:w-1/2 bg-white rounded-lg shadow-md p-6'>
          ...
          {formData.map((category) => (
            <div key={category.id} className="flex items-center mb-4">
              <Link
                to={`/jobsreview/${category.id}`} // Use the category id in the URL
        ...
        <div className="md:w-1/2 bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold mb-4 text-gray-800">About Jobs</h2>
          {formData.map((category) => (
