...
const Majors = () => {
  ...
  useEffect(() => {
    axios.get<Form[]>(`http://localhost:8080/auth/getMajors/${id}`)
         .then((response) => {
          console.log(response.data)
          setFormData(response.data)
         })
         .catch((error: any) => console.error("Error fetching majors: ", error))
  },[])
      ...
      <div className='md:w-1/2 bg-white rounded-lg shadow-md p-6'>
        ...
        {formData.map((category) => (
          <div key={category.id} className="flex items-center mb-4">
            <Link
              to={`/majorsreview/${category.id}`} // Use the category id in the URL
            ...
            >
              {category.majorName} {/* Display the category name on the button */}
            </Link>
          </div>
        ))}
      </div>
