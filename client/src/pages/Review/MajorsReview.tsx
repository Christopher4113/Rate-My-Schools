...
const MajorsReview = () => {
    ...
    const fetchData = async () => {
      try {
        const [reviews, avg, total, name] = await Promise.all([
          axios.get<Form[]>(`${serverURL}/auth/getMajorsReview/${id}`),
          axios.get(`${serverURL}/auth/getMajorsAverageRating/${id}`),
          axios.get(`${serverURL}/auth/getMajorsTotalReviews/${id}`),
          axios.get(`${serverURL}/auth/getMajorsName/${id}`)
        ]);
        ...
        setCategory(name.data.clubName);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${serverURL}/auth/postMajorsReview`, {
        major: { id: Number(id) },
        rating,
        review: reviewText,
      });
      ...
      const reviewResponse = await axios.get<Form[]>(`${serverURL}/auth/getMajorsReview/${id}`);
      const sortedData = [...reviewResponse.data].sort((a, b) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
      setFormData(sortedData);

      const avgResponse = await axios.get(`${serverURL}/auth/getMajorsAverageRating/${id}`);
      setAverageRating(avgResponse.data.averageRating);

      const totalResponse = await axios.get(`${serverURL}/auth/getMajorsTotalReviews/${id}`);
