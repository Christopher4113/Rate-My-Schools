...
const Verify = () => {
  ...
  const resendCode = async (e:any) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:8080/auth/resend",email);
        ...
        <div className="relative">
          ...
          <div id="form-container" className="bg-white p-16 rounded-lg shadow-2xl w-80 relative z-10 transform transition duration-500 ease-in-out">
            ...
            <form className="space-y-5" onSubmit={(e) => handleVerify(e)}>
              <input className="w-full h-12 border border-gray-800 px-3 rounded-lg" placeholder="Passcode" id="" name="" type="text" value={passcode} onChange={onInputChange}/>
